...
$P_t$ représente la matrice système à l'instant t. Chaque élément $p_{ij}$ de cette matrice indique la probabilité de détecter à la ligne de réponse $i$ un évènement généré au voxel $j$. Cette équation est basée sur l'algorithme OPL-EM présenté en~\ref{lab:OPLEM}. C'est cette matrice $P$ qui contient les informations du champ de mouvement. $P_t f$ correspond à la projection de l'image $f$ à l'instant $t$.  
...
\begin{figure}[h!]
	...
	\end{center}
	\caption[Index de CHO pour différentes techniques de correction du mouvement respiratoire en fonction du nombre d'itérations de la reconstruction]{Index de CHO pour différents techniques de correction du mouvement respiratoire en fonction du nombre d'itérations de l'aogorithme de reconstruction. La méthode utilisant la correction post-reconstruction est comparée avec une méthode hybride présentée dans le document en~\ref{lab:evolGating}.} 
	\label{fig:apportCHO}
\end{figure}

\todo{ajouter apportCHO}
