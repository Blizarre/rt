...
De la même manière que sur les données ne contenant que le premier niveau de décomposition, les performances obtenues pour $j=2$ montrent une dispersion très importante des points de donnée pour les images statiques et ET-IM. Le nuage de points correspondant à ET-LOR est plus proche de ceux observés précédemment. La base non corrigée est clairement en difficulté car l'apport des informations du second niveau de décomposition diminue les performances maximales du CAD en sensibilité.

En ajoutant les informations des niveaux de décomposition supérieurs, on observe une amélioration nette des performances, surtout pour la base non corrigée. Toutes les modalités bénéficient des informations ajoutées par le 4è niveau de décomposition. Cela semble indiquer que le classifieur est moins adapté pour gérer ces données. 
...
Si l'on se contente de comparer les courbes F-ROC pour un NLF donné de 9, la base d'images statiques a la meilleur sensibilté avec 58\%, suivie par ET-IM avec 53\% et ET-LOR avec 48\%. Les images non corrigées sont en net retrait avec seulement 37\% de sensibilité.
...
Les résultats des mesures de performance sur le Foie sont contrastées. Les courbes F-ROC montrent un clair avantage des techniques de correction du mouvement respiratoire, surtout pour ET-IM aux niveaux de NLF disponibles pour toutes les modalités. Par contre, les Figures de Mérite obtenues par JAFROC montrent à l'inverse que ET-IM et ET-LOR sont au même niveau que les images non corrigées du mouvement, en net retrait par rapport aux images Statiques.
