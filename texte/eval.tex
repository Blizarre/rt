...
\todo{revoir la manière dont est dite cette partie ?}
...
\todo{changer partout LNF par NFM}
...
Une seconde version de JAFROC existe avec un pouvoir statistique plus important, mais elle nécessite de disposer d'un grand nombre de cas non pathologiques. La formule est la même que la précédente (eq. \ref{eq:JAFROC1}). La seule différence est que la première sommation se fait sur l'ensemble des cas non pathologiques $N_N$ (eq. \ref{eq:JAFROC2}).
...
\todo{voir quels algos doivent préciser le nombre de classes}
...
Grâce à la maximisation de la marge, les SVM sont performants dans le cas où l'on ne dispose que de faibles quantités de données. 
\todo{citer les autres ???}
...
\todo{trouver publi utilisation LDA}
...
\todo{continuer avantages et inconvenients CAD}
