...
L'imagerie TEP permet de visualiser de manière indirecte des processus physiologiques survenant dans le corps du patient. Je vais présenter rapidement les événements qui se produisent lors d'un examen, présentés dans la figure~\ref{fig:schemaTEP}.
...
En clinique, on utilise habituellement le SUV (Standard Uptake Value), qui correspondu à une tentative de fournir des valeurs indépendantes de la dose injectée et du patient. Cela permet de réaliser des comparaisons entre des fixations radioactives de plusieurs examens, voir de plusieurs patients.
